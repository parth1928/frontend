...
const ViewStudent = () => {
    ...
    const studentType = query.get('type') || 'Regular';
    ...
    useEffect(() => {
        if (studentType === 'D2D') {
            dispatch(getUserDetails(studentID, 'D2D'));
        } else {
            dispatch(getUserDetails(studentID, address));
        }
    }, [dispatch, studentID, studentType]);
    ...
    const StudentDetailsSection = () => {
        let className = '';
        if (userDetails.sclassName && typeof userDetails.sclassName === 'object' && userDetails.sclassName.sclassName) {
            className = userDetails.sclassName.sclassName;
        } else if (typeof userDetails.sclassName === 'string') {
            className = userDetails.sclassName;
        }
        let schoolName = '';
        if (userDetails.school && typeof userDetails.school === 'object' && userDetails.school.schoolName) {
            schoolName = userDetails.school.schoolName;
        } else if (typeof userDetails.school === 'string') {
